...
function dirname() {
  ...
  if (dirname.indexOf(dirname, '.asar.unpacked') === -1) {
    dirname = dirname.replace('.asar', '.asar.unpacked');
  }
  return dirname;
}
...
function generateCommand(type, path, options) {
	...
	var command = '"' + dirname() + '\\shortcut\\shortcut.exe"' +
		' /A:' + type +
		' /F:"' + path + '"';
...
exports.query = function(path, callback) {
	exec('"' + dirname() + '/shortcut/shortcut.exe" /A:Q /F:"' + path + '"',
